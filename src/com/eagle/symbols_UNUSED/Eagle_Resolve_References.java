...
public abstract class Eagle_Resolve_References
{
	...
	protected void connectReferencesOutside(EagleSyntax syntax, EagleScope scope, AbstractToken token,
			Class<? extends DefinitionInterface> definitionClass, Class<? extends ReferenceInterface> referenceClass,
	...
	protected void connectRefs(EagleSyntax syntax, EagleScope scope, AbstractToken token, ArrayList<AbstractToken> defs,
	...
	protected ArrayList<AbstractToken> findAllInstancesOutside(AbstractToken token, Class<?> targetClass,
			Class<? extends AbstractToken> outsideClass)
	{
	...
	// Careful, recursive
	private void findEm(ArrayList<AbstractToken> tokenList, AbstractToken token, boolean optional, Class<?> targetClass,
			Class<? extends AbstractToken> outsideClass)
	{
		...
		else
		{
			...
			{
				// System.out.println("** Checking outside=" + outsideClass.getName() + "
				// token=" + token.getClass().getName());
				if (outsideClass.isAssignableFrom(token.getClass()))
				{
					// System.out.println("***** Skipping outside=" + outsideClass.getName() + "
					// token=" + token);
	...
	public void printCrossReference(PrintStream out, AbstractToken token)
	{
		...
		for (AbstractToken defToken : definitions)
		{
			...
			out.print(EagleUtilities.rj(def.getStartLine() + 1, 5) + "  " + EagleUtilities.lj(def.toString(), 20) + "  "
